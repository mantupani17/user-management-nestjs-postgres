...
export class AuthService extends BaseService {
  ...
  async getRolePermissions(roleId: number) {
    const details = instanceToPlain(
      await this.rolemodulePerService.getPermissionDetailsByRoleId(+roleId),
    )
    return (
      details &&
      details.map((i: any) => [i.permissions.permission, i.modules.module])
    )
  }
