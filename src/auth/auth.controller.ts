import { BadRequestException, Body, Controller, Get, HttpException, HttpStatus, NotFoundException, Post, Req, Request, UnauthorizedException, UseGuards } from '@nestjs/common';
import { CreateUserPayload, ForgotPasswordPayload, InitiateForgotPaswordPaylod, LoginPayload, LogoutPaylod, VerifyAccountPaylod } from '@app/user/dtos/create-user.dto';
import { JwtService } from '@app/common/jwt/jwt.service';
import { JwtAuthGuard } from '@app/common/jwt/jwt.guard';
import { CaslGuard } from '@app/common/ability/casl.guard';
import { SetAbilities } from '@app/common/decorators/ability';
...
export class AuthController {
    ...
    async signup(@Body() body: CreateUserPayload){
        const hashedPassword: string = await this.cryptoService.hashPassword(body.password)
        ...
        // send email
        await this.mailService.sendEmail( body.email, 'Signup Confirmation', `<p> Clcik <a href="${verifyUrl}">here</a> to verify your token.</p>`)
    ...
    async login(@Body() body: LoginPayload){
        const userDetails = instanceToPlain(await this.authService.findByCondition({
            email: body.email
        }))

        if (!userDetails?.length) throw new UnauthorizedException('Unathorized Access')
        
        const permission = await this.authService.getRolePermissions(userDetails[0]?.roleId || 2)
    ...
    async profile(@Request() req){
        const userDetails = instanceToPlain(await this.authService.findByCondition({status: 1, id: Number(req.user.sub)}));
        if (!userDetails?.length) throw new UnauthorizedException('Unathorized Access')
        userDetails[0]['role'] = userDetails[0]['role']['role']
    ...
    async verifyEmail(@Body() body: VerifyAccountPaylod){
        const user: any = instanceToPlain(await this.authService.findOneByCond({
        ...
        if (Number(user.verifyTokenExpirey) < Date.now()) throw new BadRequestException('Token expired, please contact with the admin.')
    ...
    async initiateForgotPassword(@Body() body: InitiateForgotPaswordPaylod){
        ...
        // send email
        await this.mailService.sendEmail( body.email, 'Forgot Password Confirmation', `<p> Clcik <a href="${verifyUrl}">here</a> to verify your token.</p>`)
    ...
    async forgotPassword(@Body() body: ForgotPasswordPayload){
        ...
        if (Number(user.forgotPasswordTokenExpirey) < Date.now()) throw new BadRequestException('Token expired, please contact with the admin.')

        const hashedPassword: string = await this.cryptoService.hashPassword(password)
        await this.authService.update(user.id, {
