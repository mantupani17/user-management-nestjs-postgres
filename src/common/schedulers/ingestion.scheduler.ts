...
export class IngTrackSchedulerService {
  ...
  constructor(
    ...
    @InjectRepository(TrackIngestion) private readonly ingRepository: Repository<TrackIngestion>,
    @InjectRepository(IngestionData) private readonly ingDataRepository: Repository<IngestionData>,
    private schedulerRegistry: SchedulerRegistry
  ){
    this.dataFilePath = path.join(__dirname, '..', '..', '..', this.configService.get<string>('upload.dataDir'));
    this.run_ = this.stringToBoolean(this.configService.get<string>('run_ingestion_cron'))
  ...
  async handleCron() {
    ...
    const records = [];
    const trackDetails = instanceToPlain(await this.ingRepository.save(
      {
        startTime: new Date(),
        status: 2,
        taskId: uuid4(),
  ...
  async processData(trackDetails:any, data: any[]) {
    try {
      ...
      await this.ingRepository.update({id: +trackDetails.id} , {
        endTime: new Date(),
        status: 1,
      })
      this.startCronJob()
