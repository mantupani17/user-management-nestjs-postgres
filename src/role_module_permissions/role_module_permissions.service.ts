...
export class RoleModulePermissionsService extends BaseService{
  constructor(
    @InjectRepository(RoleModulePermission) moduleRepository: Repository<RoleModulePermission>,
  ...
  async getPermissionDetailsByRoleId(roleId: number) {
    const userRepository = this.dataSource.getRepository(RoleModulePermission)
      const queryBuilder = await userRepository.createQueryBuilder('rmps')
      ...
      .select(["roles.role", "modules.module", 'rmps.moduleId', 'rmps.roleId', 'rmps.permissionId', 'permissions.permission'])
      .getMany();
      return queryBuilder
  }
}
