...
export class RoleModulePermissionsController {
  ...
  create(@Body() createRoleModulePermissionDto: CreateRoleModulePermissionDto) {
    return this.roleModulePermissionsService.create(createRoleModulePermissionDto);
  }

  @Get()
  findAll(@Query() q: PartialQueryOptions) {
    const { limit, skip, order, select } = q
    return this.roleModulePermissionsService.findAll({}, select, limit, skip, order );
  }
  ...
  @Patch(':id')
  update(@Param('id') id: string, @Body() updateRoleModulePermissionDto: UpdateRoleModulePermissionDto) {
    return this.roleModulePermissionsService.update(+id, updateRoleModulePermissionDto);
  }
